            ...
            <div class="px-8 py-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm p-8">
                    <form id="campaignForm" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Manfaat Program (Poin-poin)
                            </label>
                            <div id="benefitsContainer" class="space-y-3">
                                <?php 
                                if ($edit_campaign && !empty($edit_campaign['benefits'])): 
                                    foreach ($edit_campaign['benefits'] as $index => $benefit): 
                                ?>
                                <div class="flex items-center gap-2 benefit-item">
                                    <input type="text" name="benefits[]" value="<?php echo htmlspecialchars($benefit); ?>" placeholder="Contoh: Mengurangi abrasi pantai" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-primary-600 focus:ring-2 focus:ring-primary-100 outline-none transition">
                                    <button type="button" class="text-red-500 hover:text-red-700 p-2 <?php echo count($edit_campaign['benefits']) <= 1 ? 'hidden' : ''; ?>" onclick="removeBenefit(this)">
                                ...
                                ?>
                                <div class="flex items-center gap-2 benefit-item">
                                    <input type="text" name="benefits[]" placeholder="Contoh: Mengurangi abrasi pantai" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-primary-600 focus:ring-2 focus:ring-primary-100 outline-none transition">
                                    <button type="button" class="text-red-500 hover:text-red-700 p-2 hidden" onclick="removeBenefit(this)">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>
                                <?php endif; ?>
                            </div>
                            <button type="button" id="addBenefitBtn" class="mt-3 text-sm text-primary-600 font-medium hover:text-primary-700 flex items-center">
                                <i class="fas fa-plus mr-1"></i> Tambah Manfaat
                            </button>
                        </div>
    ...
    <script>
        ...
        if (addBenefitBtn && benefitsContainer) {
            addBenefitBtn.addEventListener('click', function() {
                const items = benefitsContainer.querySelectorAll('.benefit-item');
                
                const newItem = document.createElement('div');
                newItem.className = 'flex items-center gap-2 benefit-item';
                newItem.innerHTML = `
                    <input type="text" name="benefits[]" placeholder="Contoh: Mengurangi abrasi pantai" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-primary-600 focus:ring-2 focus:ring-primary-100 outline-none transition">
                    <button type="button" class="text-red-500 hover:text-red-700 p-2" onclick="removeBenefit(this)">
                        <i class="fas fa-trash"></i>
                    </button>
                `;
                benefitsContainer.appendChild(newItem);
