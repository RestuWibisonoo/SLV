...
class DonationController {
    ...
    public function processDonation() {
        ...
        // Proses donasi
        $donation_ids = $this->cart->convertToDonation([
        ...
        if ($donation_ids && is_array($donation_ids) && count($donation_ids) > 0) {
            // Ambil data donasi untuk response
            $first_donation_id = $donation_ids[0];
            $donation_data = $this->donation->getById($first_donation_id);
            $ids_string = implode(',', $donation_ids);
            
            echo json_encode([
                'success' => true,
                'message' => 'Donasi berhasil diproses',
                'donation_ids' => $ids_string,
                'donation_number' => $donation_data['donation_number'],
                'redirect_url' => 'success.php?donation_ids=' . $ids_string
            ]);
        } else {
            echo json_encode([
                'success' => false,
