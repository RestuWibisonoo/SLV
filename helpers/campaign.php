 ...
 * 
 * @param string $category_filter Filter category
 * @return int Total campaign
 */
function getTotalCampaigns($category_filter = 'all')
{
    $db = getDB();
    ...
    if ($category_filter != 'all') {
        ...
        $query .= " AND tree_type = '{$category_filter}'";
    }
 ...
 * @param int $limit Jumlah campaign per halaman
 * @param int $offset Offset data
 * @return array Array berisi list campaign
 */
function getCampaigns($category_filter = 'all', $sort_by = 'popular', $limit = 6, $offset = 0)
{
    $db = getDB();
