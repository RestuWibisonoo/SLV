...
// (Nominal donasi diganti menjadi pohon)
...
// ─── TREND POHON PER BULAN (12 BULAN TERAKHIR) ──────────────────────────────

$monthly_raw = db_query("
    SELECT 
        DATE_FORMAT(created_at, '%Y-%m') as ym,
        DATE_FORMAT(created_at, '%b') as month_label,
        COALESCE(SUM(trees_count), 0) as total
    FROM donations
    WHERE status = 'paid'
...
// ─── LAPORAN POHON TERKUMPUL ──────────────────────────────────────────────────

$finance = db_get_row("
    SELECT 
        COALESCE(SUM(CASE WHEN status = 'paid' THEN trees_count ELSE 0 END), 0) as total_paid,
        COALESCE(SUM(CASE WHEN status = 'pending' THEN trees_count ELSE 0 END), 0) as total_pending
    FROM donations
");
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6">
            ...
            <div class="stat-card bg-white rounded-2xl p-6 shadow-card">
                ...
                <p class="text-xs text-gray-400 mt-2">program penanaman</p>
            </div>
        </div>
    </div>
</section>
        ...
        <div class="border-b border-gray-200 mb-8">
            <div class="flex space-x-8 overflow-x-auto">
                ...
                <button onclick="showTab('donations')"
                    ...
                    <i class="fas fa-chart-line mr-2"></i>Trend Pohon
                </button>
        ...
        <div id="donations-tab" class="tab-content hidden">
            <div class="bg-white rounded-2xl shadow-card p-6">
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
                    <h3 class="text-lg font-bold text-gray-900">Trend Pohon 12 Bulan Terakhir</h3>
                    <div class="flex items-center space-x-2 mt-2 md:mt-0">
                        <span class="text-sm text-gray-500">Total: </span>
                        <span class="font-bold text-primary-700">
                            <?php echo number_format($total_trees_collected); ?> Pohon</span>
                    </div>
                </div>
                <div class="h-80">
                ...
                $avg_per_donor = $total_donors > 0 ? $total_trees_collected / $total_donors : 0;
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-gray-200">
                    <div class="text-center">
                        <p class="text-sm text-gray-500 mb-1">Pohon Tertinggi/Bln</p>
                        <p class="font-bold text-gray-900">
                            <?php echo number_format($max_monthly); ?> Pohon
                        </p>
                    ...
                    <div class="text-center">
                        ...
                        <p class="font-bold text-gray-900">
                            <?php echo number_format($avg_monthly, 1); ?> Pohon
                        </p>
                    ...
                    <div class="text-center">
                        ...
                        <p class="font-bold text-green-600">
                            <?php echo number_format($total_12m); ?> Pohon
                        </p>
                        <p class="text-xs text-gray-400">pohon terkumpul</p>
                    </div>
                    <div class="text-center">
                        <p class="text-sm text-gray-500 mb-1">Rata-rata/Donatur</p>
                        <p class="font-bold text-gray-900"><?php echo number_format($avg_per_donor, 1); ?> Pohon</p>
...
<section class="py-12 bg-gray-50/50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-card p-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 mb-2">Laporan Pohon Terkumpul</h2>
                    <p class="text-gray-600">Transparansi jumlah pohon dari donatur</p>
                </div>
            ...
            <div class="grid md:grid-cols-3 gap-6 mb-8">
                <div class="bg-gray-50 rounded-xl p-5">
                    <p class="text-sm text-gray-500 mb-1">Total Pohon (Termasuk Pending)</p>
                    <p class="text-2xl font-bold text-gray-900">
                        <?php echo number_format($finance_paid + $finance_pending); ?> Pohon
                    </p>
                    <p class="text-xs text-gray-400 mt-2">Per <?php echo date('d M Y'); ?></p>
                </div>
                <div class="bg-gray-50 rounded-xl p-5">
                    <p class="text-sm text-gray-500 mb-1">Pohon Terverifikasi (Paid)</p>
                    <p class="text-2xl font-bold text-gray-900">
                        <?php echo number_format($finance_paid); ?> Pohon
                    </p>
                ...
                <div class="bg-gray-50 rounded-xl p-5">
                    <p class="text-sm text-gray-500 mb-1">Pohon Tertanam</p>
                    <p class="text-2xl font-bold text-gray-900">
                        <?php echo number_format($total_trees_planted); ?> Pohon
                    </p>
                    <p class="text-xs text-green-600 mt-2">realisasi penanaman</p>
                </div>
            </div>

            <div class="border-t border-gray-200 pt-6">
                <h4 class="font-semibold text-gray-900 mb-2">Ringkasan Pohon per Campaign</h4>
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="text-left text-xs text-gray-500 uppercase border-b">
                                <th class="pb-2 pr-4">Campaign</th>
                                <th class="pb-2 pr-4 text-right">Pohon Terverifikasi</th>
                        ...
                        <tbody>
                            ...
                            $fin_rows = db_query("
                            SELECT c.title, 
                                   COALESCE(SUM(CASE WHEN d.status='paid' THEN d.trees_count ELSE 0 END),0) as total_paid,
                            ...
                            foreach ($fin_rows as $fr): ?>
                                <tr class="border-b border-gray-100">
                                    ...
                                    <td class="py-2 pr-4 text-right text-gray-700">
                                        <?php echo number_format($fr['total_paid']); ?> Pohon
                                    </td>
        ...
        new Chart(ctx2, {
            ...
            options: {
                ...
                plugins: {
                    tooltip: {
                        callbacks: {
                            label: ctx => ctx.parsed.y.toLocaleString('id-ID') + ' Pohon'
                        }
                    }
                },
                scales: {
