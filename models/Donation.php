...
class Donation
{
    ...
    public function create($data)
    {
        ...
        if ($this->conn->query($sql)) {
            ...
            if ($status === 'paid') {
                $this->confirmDonation($donation_id, true);
            }
     ...
     */
    public function confirmDonation($id, $is_new_paid = false)
    {
        $id_esc = $this->conn->real_escape_string($id);
        ...
        if ($affected > 0 || $is_new_paid) {
            ...
            if ($donation) {
                ...
                // Insert Certificate to DB
                $cert_no = $this->conn->real_escape_string($donation['certificate_number']);
                $donation_id = (int)$donation['id'];
                $donor_name = $this->conn->real_escape_string($donation['donor_name']);
                $campaign_name = $this->conn->real_escape_string($donation['campaign_title']);
                $trees_count = (int)$donation['trees_count'];
                $issued_at = date('Y-m-d');
                $issued_by = 'Sodakoh Pohon';
                
                $check_cert = $this->conn->query("SELECT id FROM certificates WHERE donation_id = {$donation_id}");
                if ($check_cert && $check_cert->num_rows == 0) {
                    $sql_cert = "INSERT INTO certificates (certificate_number, donation_id, donor_name, campaign_name, trees_count, issued_at, issued_by) 
                                 VALUES ('{$cert_no}', {$donation_id}, '{$donor_name}', '{$campaign_name}', {$trees_count}, '{$issued_at}', '{$issued_by}')";
                    $this->conn->query($sql_cert);
                }
