...
$donation_ids_str = isset($_GET['donation_ids']) ? $_GET['donation_ids'] : (isset($_GET['donation_id']) ? $_GET['donation_id'] : '');

$donationModel = new Donation();
$donations     = [];

if (!empty($donation_ids_str)) {
    $ids = explode(',', $donation_ids_str);
    foreach ($ids as $id) {
        if ((int)$id > 0) {
            $d = $donationModel->getById((int)$id);
            if ($d) $donations[] = $d;
        }
    }
}
...
if (!empty($donations)) {
    if (count($donations) == 1) {
        ...
        // Gabungkan donasi jika ada banyak
        $trees = 0;
        $amount = 0;
        $donation_numbers = [];
        ...
        foreach ($donations as $d) {
            $trees += $d['trees_count'];
            $amount += $d['amount'];
            $donation_numbers[] = $d['donation_number'];
        }
        
        $donation = [
            'donation_number'    => implode(', ', $donation_numbers),
            'campaign_title'     => 'Berbagai Program (' . count($donations) . ')',
            'trees_count'        => $trees,
            'amount'             => $amount,
            'donor_name'         => $donations[0]['donor_name'],
            'created_at'         => $donations[0]['created_at'],
            'certificate_number' => $donations[0]['certificate_number'],
                ...
                <div class="bg-gray-50 rounded-2xl p-6 mb-8 text-left">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between">
                            ...
                            <?php elseif ($donation['status'] === 'failed' || $donation['status'] === 'cancelled'): ?>
